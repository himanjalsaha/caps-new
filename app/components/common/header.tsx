    ...
    <nav className="fixed top-0 w-full z-50 bg-[#242526] border-b border-[#3E4042] px-4">
      <div className="max-w-7xl mx-auto h-14 flex items-center justify-between">
        ...
        <div className="md:hidden flex items-center">
          <button onClick={toggleDrawer} className="text-gray-300">
            {isDrawerOpen ? <Menu className="w-6 h-6" /> : <Menu className="w-6 h-6" />}
          </button>
        </div>
      ...
      {isDrawerOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-40" onClick={toggleDrawer}>
          ...
          >
            <button onClick={toggleDrawer} className="self-end text-gray-300 mb-4">
              <X className="w-6 h-6" />
            </button>
...
function NavButton({ icon, active = false, onClick, href, label }: NavButtonProps) {
  return (
    <Link href={href} passHref>
      <button
        onClick={onClick}
        className={`flex items-center space-x-2 p-2 rounded-lg hover:bg-[#3A3B3C] ${active ? 'text-purple-500' : 'text-gray-300'}`}
      >
        {icon}
        <span>{label}</span>
      </button>
